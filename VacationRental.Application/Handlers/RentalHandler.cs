...
namespace VacationRental.Application.Services
{
    internal class RentalHandler:IRentalHandler
    {
        ...
        public async Task<RequestHandler<RentalCreateResponse>> Create(RentalCreate input)
        {
            ...
            {
                return RequestHandler<RentalCreateResponse>.New().With(Error.WithMessage("Failed to Insert Rental"));
            }

            return RequestHandler<RentalCreateResponse>.New().With(new RentalCreateResponse() {RentalId = rental.Id});
        }
    }
}