...
namespace VacationRental.Api.Tests
{
    ...
    public class PostBookingTests
    {
        ...
        [Theory(Skip = "Will Test this with unit tests")]
        [MemberData(nameof(OverlappingIntervals))]
        public async Task GivenOverlappingInterval_WhenPostBooking_ThenShouldReturnErrorWithSpecificMessage(
            DateTime existingStart, DateTime existingEnd, DateTime nextStart, DateTime nextEnd)
        {
            var rentalResourceId = 
                await GivenIdForSuccessfulRentalPostWith(noOfUnits: 1,_client);
            ...
            {
                RentalId = rentalResourceId.Id,
                Nights = NoOfDaysFrom(existingStart, existingEnd),
                Start = existingStart
            };
            ...
            {
                RentalId = rentalResourceId.Id,
                Nights = NoOfDaysFrom(nextStart, nextEnd),
                Start = nextStart
            };
        ...
        private static DateTime After(DateTime date)
        {
            return date.AddDays(2);
        }
        
        private static DateTime Before(DateTime date)
        {
            return date.AddDays(-2);
        }

        private static int NoOfDaysFrom(DateTime startDate, DateTime endDate)
        {
            return (endDate - startDate).Days;
        }
        
        public static readonly object[][] OverlappingIntervals =
        {
            new object[] { SomeStartDay, SomeEndDay, Before(SomeStartDay), Before(SomeEndDay)},
            new object[] { SomeStartDay, SomeEndDay, After(SomeStartDay), Before(SomeEndDay)}
        };
    }
}
